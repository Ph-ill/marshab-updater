...
def fresh_save(now=0):
    return {'save_version':SAVE_VERSION,'last_seen_wall_ms':now,'created_wall_ms':now,'act':0,'legacy':0,'complete':False,'resources':{'o2':55,'power':30,'water':28,'food':22,'regolith':0,'population':9,'atmosphere':0,'temperature':0,'biomass':0},'caps':{'o2':120,'power':100,'water':90,'food':90,'regolith':300,'population':12,'atmosphere':1,'temperature':1,'biomass':1},'modules':{},'cooldowns':{},'unlocked_actions':['vent_co2','reroute_power','collect_ice','sift_regolith'],'unlocked_tabs':['hab','surface','comms'],'archive':['arc_system_memory_01'],'letters':[],'ending':[],'events':['evt_equipment_backup_boot'],'event_recent':['evt_equipment_backup_boot'],'away_recent':[],'counters':{'decode_count':0,'survey_count':0,'explore_count':0},'tends':{},'passport':{'visits':[],'tokens':0},'redeemed':[],'trade_seq':0,'rng':12345}
def migrate(s):
    if not isinstance(s,dict): return fresh_save(0)
    s.setdefault('save_version',1); s.setdefault('resources',{}); s.setdefault('caps',{}); s.setdefault('modules',{}); s.setdefault('cooldowns',{}); s.setdefault('unlocked_actions',['vent_co2']); s.setdefault('unlocked_tabs',['hab']); s.setdefault('archive',[]); s.setdefault('letters',[]); s.setdefault('ending',[]); s.setdefault('events',[]); s.setdefault('event_recent',[]); s.setdefault('away_recent',[]); s.setdefault('counters',{}); s.setdefault('tends',{}); s.setdefault('passport',{'visits':[],'tokens':0}); s.setdefault('redeemed',[]); s.setdefault('trade_seq',0); s.setdefault('last_seen_wall_ms',0); s.setdefault('act',0); s.setdefault('legacy',0); s.setdefault('complete',False); s.setdefault('rng',12345)
