...
function res(){let s=S.state,r=s.resources||{},rates=s.rates||{},caps=s.caps||{};
 ...
 let chip=k=>`<div class=r><b>${icon(k)} <span>${nice(k)}</span></b><strong>${k==='atmosphere'||k==='temperature'||k==='biomass'?pct(r[k]):fmt(r[k])}${S.resOpen&&caps[k]&&caps[k]<999999?' / '+fmt(caps[k]):''}</strong><em>${rate(rates[k],k)}</em></div>`;
