...
function tutorial(){if(tutDone()||!S.state)return '';let s=S.state,u=s.upgrades||{},step=tutStep();let steps=[
 {t:'Wake the hab',b:'You are the colony automation. I’ll walk you through the first real moves.',a:'Start tutorial',target:''},
 {t:'Read one resource',b:'Tap any resource chip at the top. The chip will flash its name, so you can learn the icons without opening the full panel.',a:'Waiting for resource tap…',target:'#res',wait:true},
 {t:'Run an operation',b:'Tap one ready operation, such as Vent CO₂ or Reroute Power. Operations are active commands; cooldown fill shows when they are recovering.',a:'Waiting for operation…',target:'[data-guide=ops]',wait:true},
 {t:'Build Solar Array',b:'Now spend resources on the first module. Tap Solar Array in Hab Modules. This starts the power loop that keeps the colony alive.',a:'Waiting for build…',target:'[data-build=solar_array]',wait:true},
 {t:'Visit Trade',b:'Tap the Trade tab. Trade is optional, but it teaches care-package codes and passport visits for helping other habs.',a:'Waiting for Trade tab…',target:'nav',wait:true},
 {t:'Create a care package',b:'Tap Copy Code to create a care-package code. You can share it later, or redeem carried packages from this browser.',a:'Waiting for package code…',target:'#pkg',wait:true}];
 let x=steps[step];S.tutTarget=x.target||'';let hint='';
 if(step===3){let built=['solar_array','ice_well','greenhouse'].filter(k=>(u[k]&&u[k].owned));hint=`<p class=muted>Starter loop progress: ${built.length}/3 built.</p>`}
 let primary=step===0?`<button data-tut=next>${x.a}</button>`:`<p class=wait>▶ ${x.a}</p>`;
 return `<div id=tutorial class=tut${step}><div class=tutShade></div><div class=tutCard><h2>🧭 ${x.t}</h2><p>${x.b}</p>${hint}${step?'<p class=need>Action required: use the highlighted part of the interface to continue.</p>':''}<div class=tutBtns><button data-tut=skip>Skip</button>${primary}</div></div></div>`}
function maybeTut(kind,data){
 if(tutDone())return;
 let step=tutStep();
 if((step===1&&kind==='resource')||(step===2&&kind==='action')||(step===3&&kind==='build'&&data==='solar_array')||(step===4&&kind==='tab'&&data==='comms')){setTutStep(step+1);return true}
 if(step===5&&kind==='package'){closeTut(true);return true}
 return false;
}
