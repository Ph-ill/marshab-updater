...
def start_ap(cfg):
    ...
    security = cfg.get('security','wpa2' if password else 'open')
    if security == 'open':
        password = ''
    try:
        ap.active(False)
    ...
    if password and len(password) >= 8:
        try:
            ...
            password = ''
    if not password:
        # Explicitly clear any previous secured AP config. Numeric auth/security
        # values are used because symbolic AUTH_* names vary by build.
        configured = False
        for kwargs in (
            {'essid': ssid, 'password': '', 'authmode': 0},
            {'essid': ssid, 'key': '', 'security': 0},
            {'essid': ssid, 'authmode': 0},
            {'essid': ssid, 'security': 0},
            {'essid': ssid},
        ):
            try:
                ap.config(**kwargs)
                ...
                print('open AP config attempt failed', kwargs, e)
        if not configured:
