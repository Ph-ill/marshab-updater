...
def load_config():
    defaults = {'habId':'hab_demo','ownerName':'Crew','pin':'0000','ssid':'MarsHab-Crew'}
    try:
...
def start_ap(cfg):
    ...
    ap.active(True)
    try:
        ap.config(essid=cfg.get('ssid','MarsHab'), authmode=network.AUTH_OPEN)
    except Exception:
        try:
            ap.config(essid=cfg.get('ssid','MarsHab'), security=0)
        except Exception:
            ap.config(essid=cfg.get('ssid','MarsHab'))
    try:
    ...
    for _ in range(30):
        if ap.active():
            break
        time.sleep_ms(100)
    print('MarsHab AP:', cfg.get('ssid'), ap.ifconfig())
    return ap
